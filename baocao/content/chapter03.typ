...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: ROLES],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: PERMISSIONS],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: ADMIN_ROLES],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: ROLE_PERMISSIONS],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: USERS],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: LOAIPHONG],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: PHONG],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: DICHVU],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: VOUCHERS],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: DATPHONG],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: CT_DATPHONG],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: CT_SUDUNG_DV],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: PAYMENTS],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: REFUNDS],
)
...
#figure(
  ...
  caption: [Mô Hình Mức Vật Lý: REVIEWS],
)
...
<thiet-lap-nen-tang-quan-tri-bao-mat>
...
<tai-khoan-ket-noi-csdl-cua-ung-dung>
...
<ham-kiem-tra-quyen-han>

Hệ thống triển khai hàm `F_CHECK_PERMISSION` làm nền tảng cho cơ chế RBAC (Role-Based Access Control). Mọi thủ tục và thao tác nghiệp vụ của #emph[Nhân Viên] đều cần đi qua hàm này để xác định quyền hạn tương ứng trước khi được thực hiện hoặc từ chối thao tác.
...
#figure(
  raw(read("code/ch03-seed_data-loaiphong.sql"), lang: "sql", block: true),
  caption: [Dữ Liệu Mẫu -- Các Loại Phòng.]
)
...
#figure(
  raw(read("code/ch03-seed_data-datphong.sql"), lang: "sql", block: true),
  caption: [Dữ Liệu Mẫu -- Giao Dịch Đặt Phòng.]
)

== Kết Luận Chương 3
<ket-luan-chuong-3>

Như vậy, Nhóm 02 đã hoàn thành việc xây dựng hạ tầng cơ sở dữ liệu cho bài toán #emph["Quản Lý Đặt Phòng Khách Sạn"]. Hệ thống sau khi cài đặt đã đáp ứng được các yêu cầu cốt lõi:

+ #strong[Cấu trúc lưu trữ]: Phản ánh đúng mô hình thực thể kết hợp đã thiết kế.
+ #strong[Toàn vẹn dữ liệu]: Các ràng buộc khóa chính, khóa ngoại và ràng buộc miền giá trị (CHECK constraints) đã được áp dụng chặt chẽ.
+ #strong[Dữ liệu thử nghiệm]: Bộ dữ liệu mẫu bao quát được các kịch bản nghiệp vụ chính (Đặt phòng, Thanh toán, Đánh giá).

Cơ sở dữ liệu vật lý này sẽ đóng vai trò là nền tảng trung tâm để thực hiện các thao tác xử lý dữ liệu nâng cao (Stored Procedures, Triggers, Functions, vv…) và các chính sách bảo mật sẽ được trình bày chi tiết trong #emph[Chương 4].
